...
import { Flag, Plus, Trash2, Play, Square, Share2, MapPin, X, RotateCcw, Loader2, Layers, Link as LinkIcon, CheckCircle2, ChevronRight, Activity, Edit3 } from 'lucide-react';
...
const RacesView: React.FC = () => {
  ...
  const [newRace, setNewRace] = useState<Partial<Race>>({
    name: '', distance: 0, type: RaceType.GROUP, status: RaceStatus.READY, checkpoints: [], segmentNames: []
  });
  ...
  const handleAddRace = async () => {
    ...
    try {
      await addDoc(collection(db, 'races'), { ...newRace, status: RaceStatus.READY, checkpoints: [], segmentNames: [] });
      setShowAddModal(false);
      setNewRace({ name: '', distance: 0, type: RaceType.GROUP, status: RaceStatus.READY, checkpoints: [], segmentNames: [] });
  ...
  const addCheckpoint = async (race: Race) => {
    ...
    if (!name) return;
    const distance = parseFloat(prompt("Distance cumulée depuis le départ (km) ?") || "0");
  ...
  const updateSegmentName = async (race: Race, index: number) => {
    const currentNames = race.segmentNames || [];
    const currentValue = currentNames[index] || `Tronçon ${index + 1}`;
    const newValue = prompt("Nom de ce tronçon de course ?", currentValue);
    if (newValue !== null) {
      const newNames = [...currentNames];
      while (newNames.length <= index) newNames.push("");
      newNames[index] = newValue;
      await updateDoc(doc(db, 'races', race.id), { segmentNames: newNames });
    }
  };

  const updateSegmentDiscipline = async (race: Race, index: number) => {
    const currentSegments = race.segments || [];
    const currentValue = currentSegments[index] || "Course";
    const newValue = prompt("Discipline pour ce segment ?", currentValue);
    if (newValue !== null) {
      const newSegments = [...currentSegments];
      while (newSegments.length <= index) newSegments.push("Course");
    ...
    <div className="space-y-10 pb-20">
      <header className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <button onClick={() => setShowGlobalModal(true)} className="bg-emerald-600 text-white px-6 py-4 rounded-[1.5rem] font-black flex items-center gap-3 shadow-lg shadow-emerald-100 hover:scale-105 transition-transform">
            <Layers size={20} /> Poste Multi-Course
          </button>
          <button onClick={() => setShowAddModal(true)} className="bg-blue-600 text-white px-8 py-4 rounded-[2rem] font-black flex items-center gap-3 shadow-xl shadow-blue-100 hover:scale-105 transition-transform">
      ...
      {globalCombinedPosts.length > 0 && (
        <section className="bg-emerald-50/50 p-8 rounded-[3rem] border border-emerald-100/50">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-black text-emerald-900 uppercase">Postes de Pointage Groupés</h2>
          </div>
            ...
            {globalCombinedPosts.map(post => (
              <div key={post.id} className="bg-white p-6 rounded-[2rem] border border-emerald-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => generateGlobalLink(post.id)} className="flex-1 bg-emerald-500 text-white py-3 rounded-xl font-black text-xs uppercase flex items-center justify-center gap-2 hover:bg-emerald-600">
                    <LinkIcon size={14} /> Copier le Lien
                  </button>
                  <button onClick={() => deleteGlobalPost(post.id)} className="p-3 text-slate-300 hover:text-red-500 transition-colors">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {races.map(race => {
          ...
          const disciplines = race.segments || Array(numSegments).fill("Course");
          const segmentNames = race.segmentNames || Array(numSegments).fill("");
          ...
          return (
            <div key={race.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="border-t border-slate-50 pt-8 space-y-12">
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => addCheckpoint(race)} className="bg-slate-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-blue-50 transition-colors">+ Nouveau point</button>
                  </div>
                    ...
                    {race.checkpoints.map(cp => (
                      <div key={cp.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 flex justify-between items-center hover:border-blue-200 transition-colors group relative">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => generateShareLink(race.id, cp.id)} className="p-3 bg-white text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all shadow-sm" title="Partager ce point"><Share2 size={16} /></button>
                          <button onClick={() => deleteCheckpoint(race, cp.id)} className="p-3 bg-white text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all shadow-sm" title="Supprimer ce point"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-3"><Activity size={18} /> Configuration des Tronçons</h3>
                  <div className="relative pt-8 pb-10">
                    <div className="absolute top-[33px] left-0 right-0 h-1 bg-slate-100 rounded-full"></div>
                    <div className="flex justify-between items-start relative z-10">
                      <div className="flex flex-col items-center">
                        <div className="w-4 h-4 rounded-full bg-slate-300 border-4 border-white shadow-sm"></div>
                        <span className="text-[8px] font-black text-slate-400 mt-2 uppercase">Départ</span>
                      </div>

                      <div className="flex-1 flex justify-around px-4 gap-4">
                        {disciplines.map((discipline, idx) => (
                          <div key={idx} className="flex-1 space-y-2">
                            <button onClick={() => updateSegmentName(race, idx)} className="w-full bg-white border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 rounded-xl p-3 text-center transition-all group flex flex-col items-center">
                               <p className="text-[9px] font-black text-indigo-500 uppercase truncate w-full mb-1">{segmentNames[idx] || `Tronçon ${idx+1}`}</p>
                               <Edit3 size={10} className="text-slate-300 group-hover:text-indigo-400" />
                            </button>
                            <button onClick={() => updateSegmentDiscipline(race, idx)} className="w-full bg-slate-50 border border-slate-100 hover:border-blue-500 rounded-lg py-1.5 px-2 text-center transition-all">
                               <p className="text-[7px] font-black text-blue-600 uppercase">{discipline}</p>
                            </button>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[8px] font-black text-blue-600 mt-2 uppercase">Arrivée</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-lg shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input type="text" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-black text-xl outline-none focus:border-blue-500 transition-all" value={newRace.name} onChange={e => setNewRace({...newRace, name: e.target.value})} placeholder="EX: TRAIL 15KM" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Distance (KM)</label>
                  <input type="number" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-black outline-none focus:border-blue-500 transition-all" value={newRace.distance} onChange={e => setNewRace({...newRace, distance: parseFloat(e.target.value) || 0})} />
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Type</label>
                  <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-black outline-none focus:border-blue-500 transition-all cursor-pointer" value={newRace.type} onChange={e => setNewRace({...newRace, type: e.target.value as RaceType})}>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-2xl shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-8">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Nom du poste</label>
                <input type="text" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-4 font-black text-lg outline-none focus:border-emerald-500 transition-all" value={globalName} onChange={e => setGlobalName(e.target.value)} placeholder="EX: POINT CONTRÔLE COMMUN" />
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Points à regrouper</h3>
                  <button onClick={() => setTempAssignments([...tempAssignments, {raceId: '', checkpointId: ''}])} className="text-emerald-600 text-xs font-black uppercase">+ Ajouter une course</button>
                </div>
                <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                  {tempAssignments.map((assignment, index) => (
                    <div key={index} className="flex gap-3 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      <select className="flex-1 bg-white border-none rounded-xl px-4 py-3 font-bold text-xs outline-none" value={assignment.raceId} onChange={e => { const newAs = [...tempAssignments]; newAs[index].raceId = e.target.value; newAs[index].checkpointId = ''; setTempAssignments(newAs); }}>
                        <option value="">Choisir Course...</option>
                        {races.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                      </select>
                      <select className="flex-1 bg-white border-none rounded-xl px-4 py-3 font-bold text-xs outline-none" value={assignment.checkpointId} onChange={e => { const newAs = [...tempAssignments]; newAs[index].checkpointId = e.target.value; setTempAssignments(newAs); }} disabled={!assignment.raceId}>
                        <option value="">Choisir Point...</option>
                        {races.find(r => r.id === assignment.raceId)?.checkpoints.map(cp => <option key={cp.id} value={cp.id}>{cp.name}</option>)}
                      </select>
                      <button onClick={() => setTempAssignments(tempAssignments.filter((_, i) => i !== index))} className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={16} /></button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="flex gap-4 pt-8">
                ...
                <button onClick={handleCreateGlobalPost} disabled={!globalName || tempAssignments.length < 2} className="flex-2 bg-emerald-600 text-white py-5 rounded-2xl font-black shadow-lg disabled:opacity-30">Générer le Poste Global</button>
              </div>
            </div>
          </div>
        </div>
