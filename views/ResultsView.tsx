...
/**
 * Interface ResultWithDetails étendant Participant pour inclure les données de performance.
 */
interface ResultWithDetails extends Participant {
  rank: number;
  netTime: number;
  speed: string;
...
const ResultsView: React.FC = () => {
  ...
  /**
   * Calcul des classements avec typage strict.
   */
  const processedResults: ResultWithDetails[] = useMemo(() => {
    if (!activeRace) return [];

    const results: ResultWithDetails[] = allPassages
      ...
      .map((p) => {
        ...
        const runnerPassages = allPassages.filter(pas => pas.participantId === p.participantId);
  ...
  const handleExportCSV = () => {
    ...
    const dataPourExport = processedResults.map((r) => ({
      ...
      "Temps": formatDuration(r.netTime),
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {processedResults.map((f, i) => {
                  ...
                  const segmentsData = isExpanded ? calculateSegments(f.id) : [];
