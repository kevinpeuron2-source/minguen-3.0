    ...
    return (
      <div className="fixed inset-0 bg-[#020617] flex items-center justify-center p-6 z-[200]">
        <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl w-full max-w-md text-center border-t-8 border-indigo-600">
          ...
          <p className="text-slate-400 mb-10 font-bold uppercase text-[10px] tracking-widest mt-2">Accès QG & Console Speaker</p>
          <form onSubmit={(e) => { e.preventDefault(); if(pass === config.softwarePassword) setIsAdmin(true); else alert('Code incorrect'); }} className="space-y-6">
            <input type="password" placeholder="CODE MINGUEN 3.0" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-5 text-center text-4xl font-black outline-none focus:border-indigo-500" value={pass} onChange={e => setPass(e.target.value)} autoFocus />
...
const App: React.FC = () => {
  return (
    <DatabaseProvider>
      <Router>
        <Routes>
          {/* VUES INDÉPENDANTES (Pas de Layout) */}
          ...
          {/* LOGICIEL QG (HQ) - Protégé et Intégré au Layout */}
          <Route path="/*" element={
            <SoftwareGuard>
              <Layout>
                <Routes>
